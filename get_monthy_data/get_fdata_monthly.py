...
def cols_f_to_j_fdata():
    try:
            ...
            if i == curr_year:
                ...
                try:
                    ...
                    for month in range(1, 13, 1):
                        col_names.append(date(i, month, 1).strftime('%m/%Y'))
                    df.columns = col_names
                    df = df.transpose()
                    df1 = pd.concat([df1, df], axis=0)
        return df1
        ...
        print('problem getting col: F-J ' + str(e))
    ...
    try:
        ...
        for i in range(2019, curr_year + 1, 1):
            ...
            for index in df.index:
                ...
                new_index.append(index)
            df.index = new_index
            df = df.loc[['מזה: גביית חובות מהלוואות - קרן']]
            today = date.today()
            col_names = []
        ...
        print('problem getting col: K ' + str(e))


def cols_l_to_y_fdata():
    try:
        curr_year = date.today().year
        curr_month = date.today().month
        day = date.today().day
        ...
        for i in range(2021, curr_year + 1, 1):
            ...
            for index in df.index:
                ...
                new_index.append(index)
            df.index = new_index
            df = df.loc[['גביית חובות מהלוואות - ריבית','מלווה מהמוסד לביטוח לאומי','יתר ההכנסות','הוצאות ללא מתן אשראי','משרדים','מינהליים','חברתיים','כלכליים','מערכת הביטחון','ריבית על החוב הממשלתי','ריבית וקרן למוסד לביטוח לאומי']]
            df = df.iloc[:, 5:].dropna(how='all', axis=1)
            col_names = []
        ...
        print('problem getting col: L-Y ' + str(e))
        ...
        for i in range(2019, curr_year + 1, 1):
            ...
            if i == curr_year:
                ...
                try:
                    ...
                    df = df.transpose()
                    df1 = pd.concat([df1, df], axis=0)
        df1.columns = ["local","abroad"]
        df1["גביית חובות קרן בארץ ובחול"] =df1['local']+df1['abroad']
        return df1["גביית חובות קרן בארץ ובחול"]
